...
public class FusePanel : MonoBehaviour
{
    ...
    void Start()
    {
        if (whenSolvedEnable) whenSolvedEnable.SetActive(false);
        if (fuseLight)
        {
            if (!fuseLight.gameObject.activeSelf) fuseLight.gameObject.SetActive(true);
            fuseLight.enabled = false;
            fuseLight.intensity = 0f;
            if (fuseLight.type != LightType.Directional && fuseLight.range < 8f) fuseLight.range = 10f;
        }
    }

    void Update()
    {
        if (done || !s1 || !s2 || !s3) return;
        if (s1.isOn && !s2.isOn && s3.isOn) Resolve();
    }

    void Resolve()
    {
        done = true;
        if (whenSolvedEnable) whenSolvedEnable.SetActive(true);
        if (solvedSfx) solvedSfx.Play();
        if (fuseLight)
        {
